...
def getDomainSOA(domain_name):
    ...
    try:
        soa_response = resolver.resolve(query_domain, 'SOA')
...
def getNS(domain_name):
    ...
    try:
        response_ns = resolver.resolve(domain_name, 'NS')
    ...
    for ns in response_ns:
        try:
            response_a = resolver.resolve(ns.target, 'A')
        except dns.resolver.NoAnswer:
            print("no answer returned")
        except dns.resolver.NXDOMAIN:
            print("NXDOMAIN")
...
def getDSFromNS(domain_name, name_server):
    ...
    try:
        response_ds = resolver.resolve(domain_name, 'DS')
...
def getDNSKEYFromNS(domain_name, name_server):
    ...
    try:
        response = resolver.resolve(name, rdtype, rdclass, True).response
...
def validate_zone(domain):
    ...
    for ns in child_ns:
        ...
        for dnskey in response_dnskey:
            domain_dnskey.add(dns.dnssec.key_id(dnskey))
